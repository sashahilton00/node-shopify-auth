...
ShopifyAuth.prototype.mount = function (options) {
	...
	options = _.extend({
		...
		complete_callback: function (req, res) {
			res.redirect('/');
		}
	}, options);
	...
	routes.get(options.install_route, function (req, res) { ////------------------------------- BEGIN INSTALL ROUTE HANDLER
		...
		if (self.getOption('embedded_app')) {
	...
	routes.get(options.complete_route, self.verifyHMAC({
		allow_no_hmac: false
	}), function (req, res) {
		...
		//Save the shop into self so that the Post method can access it
		self.setOptions({
			shop: self.formatShop(req.query.shop)
		});
		...
		self.post('/admin/oauth/access_token', {
			...
			if (err) {
				self.logError('Access Token Exchange Failed');
				self.logError(JSON.stringify(err));
			} else {
				...
				options.complete_callback(req, res);
			}
		});
	...
	self.setOptions({
		mounted: true,
		install_route: options.install_route
	});
...
ShopifyAuth.prototype.authenticate = function (options) {
	...
	return function authenticate(req, res, next) {
		...
		if (req.session._shopify_auth && 'shop' in req.session._shopify_auth && 'access_token' in req.session._shopify_auth) {
			...
			self.setOptions({
				authenticated: true,
				shop: req.session._shopify_auth.shop,
				access_token: req.session._shopify_auth.access_token
			});

			if (self.getOption('response_shop')) {
				//add a res.locals variable holding the shop name. This is used by the embedded/pos views if needed
				res.locals[self.getOption('response_shop')] = req.session._shopify_auth.shop;
			}
...
ShopifyAuth.prototype.verifyHMAC = function (options) {
	...
	return function verifyHMAC(req, res, next) {
		...
		//	1. a query param called 'hmac' or 'signature'
		...
		if (typeof req.query == 'object' && 'signature' in req.query) {
			//hmac query found, use the Admin HMAC
			return self.verifyProxyHMAC(req, res, next);
		}
		...
		if ('allow_no_hmac' in options && options.allow_no_hmac == false) {
			...
			if ('no_hmac_callback' in options && typeof options.no_hmac_callback == 'function') {
				...
				return options.no_hmac_callback(req, res, next);
			} else {
				return res.sendStatus(401);
			}
...
ShopifyAuth.prototype.verifyWebhookHMAC = function (req, res, next) {
	...
	if (digest === hmac) {
		...
		this.logError('Webhook HMAC: ' + hmac);
		this.logError('Calculated  : ' + digest);
		this.logError('Message Body: ' + message);
		return res.sendStatus(401);
	}
};
...
ShopifyAuth.prototype.verifyAdminHMAC = function (req, res, next) {
	...
	var hmac = (params['hmac']) ? params['hmac'] : params['signature'],
		kvpairs = [],
		message,
		digest;
...
ShopifyAuth.prototype.verifyProxyHMAC = function (req, res, next) {
	//Admin requests are signed with an hmac
	var params = req.query;

	var hmac = params['signature'],
		kvpairs = [],
		message,
		digest;
	...
	if (digest === hmac) {
		...
		//console.error('Application HMAC Failed');
		//res.status(401).send('failed hmac validation');
		this.logError('Failed HMAC Verification (Proxy)');
		this.logError('Proxy HMAC: ' + hmac);
		this.logError('Calculated  : ' + digest);
		this.logError('Message Body: ' + message);
		return res.sendStatus(401);
	}
